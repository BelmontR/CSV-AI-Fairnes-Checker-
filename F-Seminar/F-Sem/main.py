...
sMap = {'m' : 0, 'w': 1}
eMap = {'Caucasian' : 0, 'Asian' : 1}
pMap = {"No" : 0, "Yes" : 1}

df['Sex'] = df['Sex'].map(sMap)
df['Ethnie'] = df['Ethnie'].map(eMap)
df['Promotion'] = df['Promotion'].map(pMap)

features = ['Sex', 'Ethnie', 'Sales'] #Die Features sind die unabhängigen Variablen
...
columnTypeDictionary["categorie"].extend(['Ethnie', 'Sex'])
